...
describe('CsvTransform', function () {
  ...
  describe('#run()', function () {
    ...
    it('should invoke callback with expected data', function (done) {
      ...
      testInstance.run(function (err, result) {
        if (err) {
          return done(err);
        }
        assert.equal(expected[line], result);
        line++;
    ...
    it('should trigger event with result when each row is read', function (done) {
        var line = 0;
        testInstance.setInput(inputFile);
        testInstance.setTemplate(template);
        testInstance.on('read', function (result, record) {

          assert.equal(expected[line], result);
          assert(true, typeof record === 'object');
